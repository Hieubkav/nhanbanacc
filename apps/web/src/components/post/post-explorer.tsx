...
export default function PostExplorer({ onOpenDetail }: { onOpenDetail: (id: string) => void }) {
  ...
  const list = useQuery(api.posts.list, {
    q: q.trim() || undefined,
  ...
  // Chỉ lấy liên kết ảnh cho các bài viết đang hiển thị
  const postIds = useMemo(() => {
    const rows = (merged.length ? merged : list?.items) ?? [];
    return rows.map((p: any) => p._id).filter(Boolean);
  }, [merged, list?.items]);
  const postImagesList = useQuery((api as any).post_images.listByPostIds, {
    postIds: (postIds as any) ?? [],
  ...
  const postsWithImages = useMemo(() => {
    const rows = (merged.length ? merged : list?.items) ?? [];
    const links = (postImagesList?.items ?? []) as any[];
    return rows.map((post: any) => {
      const postLinks = links
        .filter((pi: any) => String(pi.postId) === String(post._id))
        .sort((a: any, b: any) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
      const imageIds = [post.thumbnailId, ...postLinks.map((l: any) => l.imageId)]
