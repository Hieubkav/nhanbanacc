...
const SEARCH_FIELDS = ["name", "email", "sdt", "diaChi", "ghiChu"] as const;
const LABEL_FIELDS = ["name", "email", "sdt"] as const;
const TOGGLE_FIELDS = ["isActive"] as const;
const UNIQUE_FIELDS = ["email", "sdt"] as const;
...
export const reorder = mutation({
  ...
  handler: async () => {
    throw new Error("customers không hỗ trợ sortOrder/reorder");
  },
});
...
export const publish = mutation({
  args: { id: v.id(TABLE) },
  handler: async () => {
    throw new Error("customers không có trường status");
  },
});
...
export const clone = mutation({
  ...
  handler: async (ctx, { id, overrides }) => {
    ...
    let candidateEmail = `${local}+copy${domain}`;
    let i = 1;
    while (await findByField(ctx, TABLE, "email", candidateEmail)) {
      candidateEmail = `${local}+copy${i}${domain}`;
      i++;
    }
    email = candidateEmail;
    // sdt mới (nếu trùng)
    let sdt = overrides?.sdt ?? src.sdt;
    let csdt = `${sdt}-copy`;
    let j = 1;
    while (await findByField(ctx, TABLE, "sdt", csdt)) {
      csdt = `${sdt}-copy${j}`;
      j++;
    }
    sdt = csdt;
    const dto = {
      ...
      sdt,
      passwordHash: overrides?.passwordHash ?? src.passwordHash,
      diaChi: overrides?.diaChi ?? src.diaChi,
      ghiChu: overrides?.ghiChu ?? src.ghiChu,
