...
import { listArgsValidator, applyFilters, applySort, paginate, matchQ, findByField, now, reorderByIndex } from "./lib/crud";

const TABLE = "post_images" as const;
...
export const list = query({ args: listArgsValidator, handler: async (ctx, args) => { const docs = await ctx.db.query(TABLE).collect(); let items = SEARCH_FIELDS.length ? docs.filter((d) => matchQ(d, args.q, SEARCH_FIELDS as any)) : docs; items = applyFilters(items, args.filters); items = applySort(items, args.sort ?? { field: "sortOrder", direction: "asc" }); return paginate(items, args.page, args.pageSize, args.cursor); } });
...
export const count = query({ args: { q: v.optional(v.string()), filters: listArgsValidator.filters! }, handler: async (ctx, args) => { const docs = await ctx.db.query(TABLE).collect(); const items = applyFilters(docs, args.filters); return { total: items.length }; } });
export const create = mutation({ args: { dto: v.object({ postId: v.id("posts"), imageId: v.id("images"), sortOrder: v.number() }) }, handler: async (ctx, { dto }) => { const id = await ctx.db.insert(TABLE, { ...dto, createdAt: now() }); return ctx.db.get(id); } });
export const update = mutation({ args: { id: v.id(TABLE), patch: v.object({ postId: v.optional(v.id("posts")), imageId: v.optional(v.id("images")), sortOrder: v.optional(v.number()) }) }, handler: async (ctx, { id, patch }) => { await ctx.db.patch(id, patch as any); return ctx.db.get(id); } });
export const guardedUpdate = mutation({ args: { id: v.id(TABLE), expectedUpdatedAt: v.number(), patch: v.object({}) }, handler: async () => { throw new Error("post_images không có updatedAt nên không hỗ trợ guardedUpdate"); } });
...
export const bulkUpdate = mutation({ args: { ids: v.array(v.id(TABLE)), patch: v.object({ postId: v.optional(v.id("posts")), imageId: v.optional(v.id("images")), sortOrder: v.optional(v.number()) }) }, handler: async (ctx, { ids, patch }) => { for (const id of ids) await ctx.db.patch(id, patch as any); return { success: true, count: ids.length }; } });
export const toggle = mutation({ args: { id: v.id(TABLE), field: v.string() }, handler: async () => { throw new Error("post_images không có boolean để toggle"); } });
export const reorder = mutation({ args: { id: v.id(TABLE), toIndex: v.optional(v.number()), beforeId: v.optional(v.id(TABLE)), afterId: v.optional(v.id(TABLE)), postScopeId: v.optional(v.id("posts")) }, handler: async (ctx, args) => reorderByIndex(ctx, TABLE, args.id, { toIndex: args.toIndex, beforeId: args.beforeId, afterId: args.afterId, scope: args.postScopeId ? { field: "postId", value: args.postScopeId } : undefined } as any) });
export const publish = mutation({ args: { id: v.id(TABLE) }, handler: async () => { throw new Error("post_images không có status"); } });
